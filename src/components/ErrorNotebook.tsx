...
export const ErrorNotebook = ({ subjects, errors, setErrors, onBack }: ErrorNotebookProps) => {
  ...
  useEffect(() => {
    const savedErrors = localStorage.getItem('errorNotebook');
    if (savedErrors) {
      setErrors(JSON.parse(savedErrors).map(error => ({
        ...error,
        date: new Date(error.date),
        reviewDates: error.reviewDates.map((date: string) => new Date(date))
      }));
    }
  }, [setErrors]);

  // Salvamento automático
  const saveErrors = debounce((updatedErrors: ErrorEntry[]) => {
    localStorage.setItem('errorNotebook', JSON.stringify(updatedErrors));
    setShowSaved(true);
